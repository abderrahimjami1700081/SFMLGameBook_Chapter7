...
{
	std::ifstream file;
	file.open("Resources/Charater/Player.char");
	if (file.is_open())
	{
		std::cout << "!Failed to open Player.char" << std::endl;
	}
	...
	while (std::getline(file, line))
	{
		...
		else if (type == "Spritesheet")
		{
			std::string path;
			keystream >> path;
			m_spriteSheet.LoadSheet("Resources/Spritesheets/" + path);
		}
...
void Character::OnEntityCollision(EntityBase * l_collider, bool l_attack)
{
}
...
void Character::Update(float l_dT)
{
	...
	if (m_attackAABB.width != 0 &&
		m_attackAABB.height != 0)
	{
		UpdateAttackAABB();

	}
	if (GetState() == EntityState::Dying &&
		GetState() != EntityState::Attacking && GetState() != 
		EntityState::Hurt)
	{
...
void Character::UpdateAttackAABB()
{
	m_attackAABB.left =
		...
		(m_AABB.left - m_attackAABB.width) - m_attackAABBoffset.x :
			(m_attackAABB.left + m_AABB.width) + m_attackAABBoffset.x);
