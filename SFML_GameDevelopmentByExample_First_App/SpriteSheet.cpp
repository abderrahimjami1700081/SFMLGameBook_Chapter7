...
void SpriteSheet::ReleaseSheet()
{
	...
	//While the unorderedmap is not empty ... 
	while (m_animations.begin() != m_animations.end())
	{
		// Delete the texture and the entry in the unordered map
	...
	if (sheet.is_open())
	{
		...
		while (std::getline(sheet, line))
		{
			...
			else if (type == "Animation")
			{
				...
				{
					std::cerr << "! Unkown animation type: "
						<< m_animType << std::endl;
					continue;
				}
				keystream >> *anim;
...
bool SetAnimation(const std::string & l_name, const bool & l_play, const bool & l_loop)
{
	return false;
}
